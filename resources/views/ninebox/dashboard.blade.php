<x-app-layout>
      ...
      <div class="flex flex-col lg:flex-row gap-8">
        ...
        >
          ...
          <div class="p-7 space-y-7">
            ...
            <section aria-label="Filtro de Fecha por año y mes" class="flex flex-wrap items-center gap-3 mb-6">
              @if ($usuario->esJefe())
                <div>
                  <label for="filtro-anio" class="block text-sm font-medium text-gray-700 dark:text-white mb-1">Año</label>
                  <select id="filtro-anio" x-model="anioSeleccionado"
                ...
                <div>
                  ...
                  <select id="filtro-mes" x-model="mesSeleccionado"
                          class="rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900/70 dark:text-black
                                 focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <button id="btn-aplicar-filtro-fecha" onclick="filtrarPorFecha()" class="btn btn-primary btn-sm">
                  Aplicar
                </button>
              @endif
            </section>
        ...
        <div class="flex-1">
          <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-5 border border-gray-200 dark:border-gray-700">
            <div class="relative w-full mx-auto" style="max-width: 96%;">
              ...
              {{-- Botones sobre cada cuadrante (se mantienen atributos y posiciones) --}}
              @for ($i = 1; $i <= 9; $i++)
                @php
                  $positions = [
                ...
                <button type="button" class="cuadrante-btn btn-surface"
                        ...
                        data-cuadrante="{{ $i }}"
                        title="{{ $cuadrantesData[$i]['title'] ?? 'Cuadrante' ?? '' ?? '' }}"
                        aria-label="Ver empleados en cuadrante {{ $i }}">
                  @if(($asignacionesActuales->get($i, collect())->count()) > 0)
                    <div class="cuadrante-badge">{{ ($asignacionesActuales->get($i, collect())->count()) }}</div>
                  @endif
                </button>
              @endfor
  ...
  {{-- CSS (sin cambios funcionales, copiado como estaba) --}}
  <style>
    /* ---------- Ninebox: CSS refactorizado y modular ---------- */
    /* Reemplaza tu <style> por este bloque. Mantiene todas las clases e IDs existentes. */
    ...
    :root{
      ...
      --glass-0: rgba(255,255,255,0.06);
      --glass-1: rgba(255,255,255,0.04);

      --shadow-soft: 0 8px 18px rgba(2,6,23,0.10);
      --shadow-strong: 0 18px 48px rgba(2,6,23,0.18);
      --radius-lg: .95rem;
      --radius-md: .9rem;
      --anim-fast: .12s;
    }
    ...
    *{box-sizing:border-box}
    ...
    .btn-danger{
      color:#fff;
      background-image: linear-gradient(90deg, #ef4444 0%, var(--danger-red) 100%);
      border-color: rgba(0,0,0,0.06);
    }
    ...
    /* btn-close: estilizado especial (mantener look claro/oscuro) */
    .btn-close{
      color:#fff !important;
      background: rgba(255,255,255,0.12);
    ...
    /* Badge del cuadrante (misma estética que primario) */
    .cuadrante-badge{
      ...
      box-shadow:0 4px 14px rgba(2,6,23,.12); user-select:none; pointer-events:none; transform: translateZ(0);
    }
    ...
    /* reusar la misma gradiente para header */
    ...
    /* ===== Listas del modal: items empleados ===== */
    .lista-empleado{
    ...
    /* Sticky sidebar solo en pantallas grandes */
    ...
    /* Badge / layout tweaks */
    .depto-header { list-style: none; padding-left: 0; }
    .depto-header + .lista-empleado { margin-top: .25rem; }

    /* pequeños ajustes visuales para btn-close icon */
    #btn-cerrar-modal{ line-height:0; }
    #btn-cerrar-modal svg{ width:20px; height:20px; display:block; transform:none; }
  </style>

  {{-- JavaScript (refactorizado y encapsulado) --}}
  <script>
    /* Variables públicas necesarias por la plantilla (se mantienen globales) */
    @php
      $authId = auth()->id();
    ...
    (function(){
      ...
      // Datos server->cliente serializados
      ...
      // Cuadrantes: mantenemos tu texto original; lo convertimos a objeto accesible
      ...
      /* ---------- Render listado para superadmin (solo-lectura) ---------- */
      ...
      function limpiarSessionStorage(){ sessionStorage.removeItem(storageKey('rendimientosAsignados')); }

      /* ---------- DOM utilities y render ---------- */
      ...
      function mostrarModal(cuadrante){
        cuadranteActual = Number(cuadrante);
        const data   = cuadrantesData[cuadranteActual] || {title:'Cuadrante',desc:''};
      ...
      function asignarEmpleado(usuarioId){
        if (cuadranteActual === null) return;
      ...
      async function guardarEvaluacion(){
        ...
        }catch(e){ alert('Error al guardar evaluación: '+(e?.message || e)); }
      }
      ...
      async function filtrarPorFecha(){
        try{
          ...
          actualizarEstadisticas('recalcular');
          guardarEnSessionStorage();
        }catch(e){ alert('Error al filtrar: '+(e?.message || e)); }
      }

      /* ---------- Inicialización ---------- */
      document.addEventListener('DOMContentLoaded', ()=>{
        // Inicializar avance-bar
        const bar = document.getElementById('avance-bar');
        const pct = @json($pct);
        if (bar){ bar.style.width = pct + '%'; bar.textContent = pct + '%'; }

        // Delegación/captura de botones cuadrante
        ...
        // Evitar menú contextual en la imagen
        const img = document.getElementById('ninebox-img');
        if (img) img.addEventListener('contextmenu', e=>{ e.preventDefault(); });

        // Cargar datos guardados en session
        cargarDesdeSessionStorage();

        // Calcular badges iniciales a partir de rendimientosAsignados
        recalcularBadges();
      });

      /* ---------- Exponer funciones que usa el HTML inline ---------- */
      window.filtrarPorFecha = filtrarPorFecha;
      window.guardarEvaluacion = guardarEvaluacion;
      // También exponemos por si algo externo requiere abrir/cerrar modal/acciones
      window.mostrarModal = mostrarModal;
      window.cerrarModal = cerrarModal;
      window.asignarEmpleado = asignarEmpleado;
