...
@section('title', 'Crear Dueño | NineBox')
...
@section('header')
    <x-breadcrumb :items="[
        ...
        ['label' => 'Dueño', 'url' => null],
    ]" />
    <h2 class="font-semibold text-xl text-ink leading-tight">
        {{ __('Crear Dueño') }} — {{ $empresa->nombre }}
    </h2>
@endsection
...
@section('content')
    <div class="py-12 bg-surface min-h-screen">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="card">
                <form action="{{ route('admin.usuarios.store-tipo', [$empresa, 'dueno']) }}" method="POST">
                    @csrf
                    <div class="mb-6">
                    ...
                    <div class="mb-6">
                        <label for="correo" class="form-label">{{ __('Correo') }} <span
                    ...
                    <div class="flex items-center justify-end space-x-3 pt-6 border-t border-border">
                        ...
                        <button type="submit" class="btn-primary">{{ __('Crear Dueño') }}</button>
                    </div>
                </form>
            </div>
        </div>
