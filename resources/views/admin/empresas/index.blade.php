...
@section('title', 'Empresas | NineBox')

@section('header')
    <x-breadcrumb :items="[['label' => 'Empresas', 'url' => null]]" />
    <div class="flex justify-between items-center">
        <h2 class="font-semibold text-xl text-ink leading-tight">
            {{ __('Empresas') }}
        </h2>
        <a href="{{ route('admin.empresas.crear') }}" class="btn-primary">
            {{ __('Nueva empresa') }}
        </a>
    </div>
@endsection
            ...
            <div class="bg-canvas border border-border rounded-lg overflow-hidden shadow-card">
                @if ($empresas->isEmpty())
                    <div class="p-12 text-center">
                        ...
                        <p class="mt-1 text-sm text-ink-2 mb-6">
                            {{ __('Crea la primera empresa para comenzar a administrar el sistema.') }}</p>
                        <a href="{{ route('admin.empresas.crear') }}" class="btn-primary">
                            {{ __('Crear la primera') }}
                        </a>
                    </div>
                                    ...
                                    <tr class="hover:bg-surface/50 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex justify-end items-center space-x-3">
                                                ...
                                                <form action="{{ route('admin.empresas.toggle', $empresa) }}"
                                                    method="POST" class="inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
                                                        class="text-sm font-medium focus:outline-none bg-transparent border-0 cursor-pointer {{ $empresa->activa ? 'text-danger hover:text-red-700' : 'text-success hover:text-green-700' }}">
